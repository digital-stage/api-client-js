...
const MediasoupProvider = (props: {
  ...
  /** ****
   * TODO: Each client is distributing!!!!! We need to use local stores for this
   */
  /*
  useEffect(() => {
    if (apiConnection && ready && stage) {
      if (localDevice?.sendVideo && localDevice?.inputVideoDeviceId) {
        ...
        navigator.mediaDevices
          ...
          .then((videoStream) => {
            report(
              `Producing ${videoStream.getTracks().length} video track(s)`
            );
            return Promise.all(
              videoStream.getVideoTracks().map((videoTrack) =>
                // eslint-disable-next-line promise/no-nesting
                produce(videoTrack)
                  .then((producer) => {
                    setVideoProducers((prev) => [...prev, producer]);
                    return producer;
                  })
                  .catch((error) => {
                    reportError('Stopping track due to an error');
                    // videoTrack.stop();
                    throw error;
                  })
              )
        ...
        return () => {
          // Stop producing all
          setVideoProducers((prev) => {
            report('Stopping ALL video producers');
            prev.map((producer) => {
              return stopProducing(producer)
                .then((stoppedProducer) => stoppedProducer.track?.stop())
  ...
  useEffect(() => {
    if (apiConnection) {
      // Sync producers with local video tracks
      const mediasoupTracks = localVideoTracks
        .filter((track) => track.producerId)
        .map((track) => track as MediasoupLocalVideoTrack);
      videoProducers
        .filter(
          (producer) =>
            !mediasoupTracks.find((track) => track.producerId === producer.id)
        )
        .forEach((producer) => {
          report(`Adding local video track for producer ${producer.id}`);

          apiConnection.emit(ClientDeviceEvents.CreateLocalVideoTrack, {
            type: 'mediasoup',
            producerId: producer.id,
      ...
      mediasoupTracks
        .filter(
          (track) =>
            !videoProducers.find((producer) => producer.id === track.producerId)
        )
        .forEach((track) => {
          report(`Removing local video track ${track._id}`);
          apiConnection.emit(
  ...
  }, [apiConnection, videoProducers.length, localVideoTracks.length]);
   */

...
const RenderStageMember = (props: { id: string }) => {
    ...
    const localDevice = useStageSelector<Device | undefined>((state) =>
        state.globals.localDeviceId ? state.devices.byId[state.globals.localDeviceId] : undefined
    )
    const stageMember = useStageSelector<StageMember>((state) => state.stageMembers.byId[id])
    const customStageMemberVolume = useStageSelector<CustomStageMemberVolume | undefined>((state) =>
        localDevice &&
        stageMember &&
        state.customStageMemberVolumes.byDeviceAndStageMember[localDevice._id][stageMember._id]
...
const SpacialAudioRendererProvider = (props: { children: React.ReactChildren }) => {
    const { children } = props
    const audioRef = useRef<HTMLAudioElement>()

    const stageMemberIds = useStageSelector<string[]>((state) => state.stageMembers.allIds)
